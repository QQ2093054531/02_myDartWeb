...
let modulePaths = {
 ...
 "web/fetch-data": "fetch-data.sound.ddc"
};
if(!window.$dartLoader) {
   window.$dartLoader = {
     appDigests: _currentDirectory + 'fetch-data.digests',
...
define("fetch-data.dart.bootstrap", ["web/fetch-data", "dart_sdk"], function(app, dart_sdk) {
  ...
  (app.web__fetch_data || app.fetch$45data).main();
  var bootstrap = {
  ...
  dart_sdk.dart.trackLibraries("fetch-data.dart.bootstrap", {
    "fetch-data.dart.bootstrap": bootstrap
  }, '');
  return {
    bootstrap: bootstrap
  };
